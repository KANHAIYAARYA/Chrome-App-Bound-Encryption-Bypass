    ...
    static const std::vector<ExtractionConfig> configs = {
        ...
        {"Web Data", "autofill", "SELECT name, value FROM autofill;",
         ...
         [](sqlite3_stmt *stmt, const auto &key, const auto &state) -> std::optional<std::string>
         {
             ...
             try
             {
                 return "  {\"field_name\":\"" + Utils::EscapeJson(name) +
        ...
        {"History", "history", "SELECT url, title, visit_count, last_visit_time FROM urls;",
         ...
         [](sqlite3_stmt *stmt, const auto &key, const auto &state) -> std::optional<std::string>
         {
             ...
             sqlite3_int64 last_visit_time = sqlite3_column_int64(stmt, 3);
             if (!url || !title)
                 return std::nullopt;
             try
             {
                 // Convert last_visit_time (microseconds since Windows epoch) to readable format
                 std::string last_visit_str = std::to_string(last_visit_time);
                 return "  {\"url\":\"" + Utils::EscapeJson(url) +
                        "\",\"title\":\"" + Utils::EscapeJson(title) +
                        "\",\"visit_count\":" + std::to_string(visit_count) +
                        ",\"last_visit_time\":\"" + Utils::EscapeJson(last_visit_str) + "\"}";
             }
             catch (...)
             {
                 return std::nullopt;
